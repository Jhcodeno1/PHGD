...
class Data(object):
    ...
    def load_data(self):
        ...
        # train_path = self.path + "/28train.txt"
        # test_path = self.path + "/28test.txt"
    ...
    def get_train_nodes(self):
        ...
        提取训练集中所有涉及的节点（用户和物品）
        Returns:
            train_nodes: 包含所有训练集用户和物品节点的集合
        """
        # 用户节点
        train_users = set(self.train_user)
        # 物品节点（物品ID需要偏移 num_users，保证唯一性）
        train_items = set(self.train_item + self.num_users)
        # 合并用户节点和物品节点
        train_nodes = train_users.union(train_items)
